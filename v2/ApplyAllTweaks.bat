@echo off
title Yamatos by @xehn1337
color 4
cls
echo Making System Restore point just incase you forgot
echo.
wmic /namespace:\\root\default path systemrestore call createrestorepoint "Yamatos", 100, 12 >nul 2>&1
...
echo What is your System Type:
    call :MsgBox "Are you a Desktop user?"  "VBYesNo+VBQuestion" "@xehn1337 Yamatos"
    if errorlevel 7 (
        echo goto:LaptopPC 
...
@echo off
    ...
    call :MsgBox "Would you like Full Screen Exclusive? lower input delay & better performance but 3 second Alt-tab (black screen)"  "VBYesNo+VBQuestion" "@xehn1337"
    if errorlevel 7 (
        echo NO - don't 
    ) else if errorlevel 6 (
        echo YES - Enable
...
if "%ERRORLEVEL%"=="0"     call :MsgBox "Would you like to close Chrome Browser to clear cache & Temp Files?"  "VBYesNo+VBQuestion" "@xehn1337"
    if errorlevel 7 (
        goto:continueclean
    ) else if errorlevel 6 (
        echo YES - Enable
...
cls
Echo Applying xehn1337's PowerPlan
echo.
...
:smol
if not exist "%USERPROFILE%\Desktop\YamatosOptionalTweaks" mkdir %USERPROFILE%\Desktop\YamatosOptionalTweaks
explorer.exe %USERPROFILE%\Desktop\YamatosOptionalTweaks
...
echo.
SET msgboxTitle=Yamatos by @xehn1337
SET msgboxBody=Yamatos Tweaks Have been Applied, Please Restart your PC & Leave a feedback on the discord <3
...
@echo off
    ...
    call :MsgBox "Would you like to Restart?"  "VBYesNo+VBQuestion" "@xehn1337"
    if errorlevel 7 (
